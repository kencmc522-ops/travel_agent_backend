...
const CACHE = globalThis.__TB_V4_20_CACHE__ || (globalThis.__TB_V4_20_CACHE__ = new Map());
...
export default async function handler(req,res){
  ...
  try{
    ...
    return res.status(200).json({...data, _cache:'MISS', _ver:'V4.20 HoverRestore+Refresh 1正5負'});
  }catch(e){
...
async function doFetch(region,type,platforms,budget,weights,famousOnly,minReviews){
  ...
  async function enrich(places, kind){
    ...
    const enriched = await Promise.all(places.map(async (p)=>{
      ...
      const seenNeg=new Set(); let neg=[]; for(const r of negCombined){ const k=(r.author_name||'')+(r.text||'').slice(0,15); if(!seenNeg.has(k)){ seenNeg.add(k); neg.push(r);} if(neg.length>=10) break; }
